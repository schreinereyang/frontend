    ...
    <>
      <div className="flex min-h-screen bg-[#0b0f1a] text-white">
        <aside
          ...
          className={`bg-[#0b0f1a] border-r border-purple-800 p-4 transition-all duration-300 ${
            sidebarOpen ? 'w-56' : 'w-16'
          } overflow-hidden fixed h-screen z-10`}
        >
          ...
          <nav className="space-y-6 mt-8">
            {menu.map((m, i) => (
              <Link
                key={i}
                href={m.href}
                className={`flex items-center space-x-3 transition-colors hover:text-purple-400 ${
                  router.pathname === m.href ? 'text-purple-400 font-semibold' : ''
                }`}
              >
